...
Bicycle::Bicycle(const second_order_matrix_t& M, const second_order_matrix_t& C1,
    ...
    initialize_state_space_matrices(false); // delay computation of state space matrices until when v, dt are set
...
Bicycle::state_t Bicycle::x_next(const Bicycle::state_t& x, const Bicycle::input_t& u) const {
    if (m_recalculate_state_space) {
        initialize_state_space_matrices(true);
    }
...
Bicycle::auxiliary_state_t Bicycle::x_aux_next(const state_t& x, const auxiliary_state_t& x_aux) const {
    if (m_recalculate_moore_parameters) {
        set_moore_parameters();
    }
 ...
 * If calculate_state_space is false, state space computation will be deferred.
 */
void Bicycle::initialize_state_space_matrices(bool calculate_state_space) {
    m_A.setZero();
    m_B.setZero();
    m_Ad.setZero();
    m_Bd.setZero();
