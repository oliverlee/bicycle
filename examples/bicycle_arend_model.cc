...
int main(int argc, char* argv[]) {
    ...
    std::cout << "simulating Arend's simplified bicycle model at constant speed..." << std::endl;
    x = x0;
    for (auto& state: system_state) {
        bicycle.set_output_element(z, model_t::output_index_t::steer_angle,
                model_t::get_state_element(x, model_t::state_index_t::steer_angle)  + r0(gen));
        bicycle.set_output_element(z, model_t::output_index_t::steer_rate,
                model_t::get_state_element(x, model_t::state_index_t::steer_rate) + r1(gen));
